...
type Edge = { a: number; b: number; dist: number; sameCluster: boolean };
const PROX_EDGES: Edge[] = (() => {
  const out: Edge[] = [];
  for (let i = 0; i < DOTS.length; i++) {
    for (let j = i + 1; j < DOTS.length; j++) {
      const dist = Math.hypot(DOTS[i].x - DOTS[j].x, DOTS[i].y - DOTS[j].y);
      if (dist <= PROX_RADIUS) {
        out.push({ a: i, b: j, dist, sameCluster: DOTS[i].cluster === DOTS[j].cluster });
      }
    }
  }
  return out;
...
const KNN_EDGES: { a: number; b: number; cluster: number }[] = (() => {
  const seen = new Set<string>();
  const out: { a: number; b: number; cluster: number }[] = [];
  for (const d of DOTS) {
    const dists = DOTS
      .filter(o => o.id !== d.id)
    ...
    for (const n of dists) {
      ...
      seen.add(key);
      if (n.cluster === d.cluster) out.push({ a: d.id, b: n.id, cluster: d.cluster });
    }
  }
  return out;
})();

export function ProximityFog() {
  return (
    <div
    ...
    >
      ...
      >
        ...
        >
          ...
          <span style={{ opacity: 0.5 }}>D-256 · N={NUM_DOTS} · R≤0.18</span>
        </div>
        ...
        <div className="absolute" style={{ top: 28, left: 0, right: 0, bottom: 0 }}>
          ...
          {/* PROX FOG: every pair within radius R, drawn very faint. Cluster cores
              become dense webs, sparse strands trail between clusters. Then a
              sharper k-NN layer in cluster colors sits on top for the dominant
              read. Two-layer depth without losing topology. */}
          <svg className="absolute inset-0 w-full h-full pointer-events-none">
            {PROX_EDGES.map((e, i) => {
              const a = DOTS[e.a];
              const b = DOTS[e.b];
              const op = (1 - e.dist / PROX_RADIUS) * 0.35;
              return (
                <line
                  key={`p-${i}`}
                  x1={`${a.x * 100}%`}
            ...
            {KNN_EDGES.map((e, i) => {
              ...
              return (
                <line
                  ...
                  x2={`${b.x * 100}%`}
                  y2={`${b.y * 100}%`}
                  stroke={CLUSTER_COLORS[e.cluster]}
                  strokeOpacity={0.85}
                  strokeWidth={1.6}
                />
              );
